-- Pulse Engine master schema
-- Root-level single source of truth for PostgreSQL bootstrap and compatibility setup.
--
-- Merged sources:
--   - backend/sql_schema.sql
--   - backend/postgresql_consolidated.sql (byte-identical duplicate of backend/sql_schema.sql)
--   - backend/sql_migrations/001_subscription_conversation_and_seats.sql
--   - backend/sql_migrations/002_usage_idempotency_and_canonical_conversation.sql
--   - backend/sql_migrations/003_oauth_states_link_user_id.sql
--   - backend/shared/db/bootstrap_microservices.sql
--   - backend/shared/db/migrate_ai_vector_optimization.sql
--   - backend/shared/db/migrate_company_identity_normalization.sql
--   - backend/shared/db/migrate_customer_company_name_normalization.sql
--
-- Execution order:
--   1. Microservice schemas / namespaces
--   2. Extensions and compatibility guards
--   3. Table definitions and inline primary / unique / foreign-key constraints
--   4. Secondary ALTER TABLE compatibility patches
--   5. Indexes and row-level-security policies
--   6. Seed / bootstrap data
--   7. Legacy-to-current compatibility migrations for existing databases
...
-- ============================================================================
-- Extensions and Compatibility Guards
-- ============================================================================
...
-- These blocks preserve behavior from the standalone migration files for
-- existing databases while remaining safe on fresh installs.
-- ============================================================================

-- Keep company_settings as pure configuration metadata and move any misplaced
-- tenant identity back onto companies.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
-- Normalize legacy company column names onto the canonical customer_company_name field.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
-- Backfill billing limits for pre-existing subscriptions and drop the legacy column.
...
-- Canonicalize historical usage_ledger rows to the unified conversation_message type.
