...
async def _process_incoming_message(
    ...
    try:
        ...
        use_conversation_engine = await company_uses_conversation_engine(db, company_id)
        workflow = await orchestrate_message_workflow(
            MessageWorkflowRequest(
                ...
                suppress_response_generation=use_conversation_engine,
            ),
            db=db,
        )
        logger.info(
        ...
        if use_conversation_engine:
            try:
                ...
                # WhatsApp / Meta channels don't have a widget session_id; the
                # conversation_id is the durable identifier the engine should
                # bind history to.
                ...
                logger.info(
                    ...
                    "company_id=%s channel=%s turn_id=%s trace_id=%s",
                    _elapsed_ms(engine_started_at), convo_id, company_id, channel,
                    engine_result.turn_id, trace_id,
                )
