...
from services.ai_service.kb_summary_service import enqueue_kb_summary_job
...
async def create_kb_doc(request: Request):
    ...
    await enqueue_kb_summary_job(db, company_id=cid, kb_id=doc_id, content=content)
...
async def update_kb_doc(doc_id: str, request: Request):
    ...
    # Re-summarise only when the content was part of the update — title-only
    # or tag-only edits don't need a fresh summary.
    if "content" in safe_body:
        await enqueue_kb_summary_job(
            db, company_id=cid, kb_id=doc_id, content=str(safe_body.get("content") or "")
        )
    return r(await db.fetchrow("SELECT * FROM knowledge_base WHERE id=$1", doc_id))
