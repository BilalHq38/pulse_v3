...
_PRICE_RE = re.compile(r"\$?\s*\d[\d,]*\.?\d*\s*(?:USD|EUR|GBP|INR|PKR|AED|SAR)?", re.IGNORECASE)
...
def _looks_like_product_name(candidate: str) -> bool:
    ...
    multi-word, contains a digit, or has internal uppercase (CamelCase). Bare
    single common words are sentence starters and aren't worth flagging."""
    if " " in candidate:
        return True
    if any(ch.isdigit() for ch in candidate):
        return True
    # Internal uppercase: present after the first character in the raw form.
    # We get the lowercased form here, so use a separate check below in
    # _check_grounding by scanning the original answer's matches.
    return False
