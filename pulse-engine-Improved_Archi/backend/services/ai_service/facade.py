...
async def analyze_sentiment(text: str, db=None, company_id: str = "", **kwargs) -> dict:
    try:
        ...
        logger.warning(
            "ai_facade analyze_sentiment local failed company_id=%s error=%s",
            company_id or "", exc.__class__.__name__,
        )
        return _safe_sentiment_default(text)
...
async def classify_intent(text: str, db=None, company_id: str = "", **kwargs) -> dict:
    try:
        ...
        logger.warning(
            "ai_facade classify_intent local failed company_id=%s error=%s",
            company_id or "", exc.__class__.__name__,
        )
        return _safe_intent_default(text)
...
async def generate_lead_score(
    ...
    try:
        return await _local_generate_lead_score(
            safe_lead, db=db, company_id=company_id, count_against_budget=count_against_budget,
        )
    except Exception as exc:
        logger.warning(
            "ai_facade generate_lead_score local failed company_id=%s error=%s",
            company_id or lead_data.get("company_id", ""), exc.__class__.__name__,
        )
...
async def generate_nurture_message(
    ...
    try:
        return await _local_generate_nurture_message(
            safe_lead, stage, company_context=company_context, db=db, company_id=company_id,
        )
    except Exception as exc:
        logger.warning(
            "ai_facade generate_nurture_message local failed company_id=%s error=%s",
            company_id or lead_data.get("company_id", ""), exc.__class__.__name__,
        )
...
async def auto_score_and_nurture_lead(lead_data: dict, db=None, company_id: str | None = None) -> dict:
    ...
    try:
        ...
        logger.warning(
            "ai_facade auto_score_and_nurture_lead local failed company_id=%s error=%s",
            company_id or lead_data.get("company_id", ""), exc.__class__.__name__,
        )
...
async def update_customer_memory(
    ...
    try:
        return await _local_update_customer_memory(
            customer_id, new_messages, previous_summary, recent_sentiment,
            db=db, company_id=company_id,
        )
    except Exception as exc:
        logger.warning(
            "ai_facade update_customer_memory local failed company_id=%s error=%s",
            company_id or "", exc.__class__.__name__,
        )
...
async def summarize_conversation(messages: list, db=None, company_id: str = "") -> str:
    try:
        ...
        logger.warning(
            "ai_facade summarize_conversation local failed company_id=%s error=%s",
            company_id or "", exc.__class__.__name__,
        )
...
async def generate_product_description(
    ...
    try:
        return await _local_generate_product_description(
            name,
            company_id=company_id, product_title=product_title, product_type=product_type,
            category=category, price=price, price_currency=price_currency,
            images=images, engines=engines, db=db,
        )
    except Exception as exc:
        logger.warning(
            "ai_facade generate_product_description local failed name=%s error=%s",
            name, exc.__class__.__name__,
        )
        return _safe_product_description_default(
            name, product_title=product_title, product_type=product_type,
            category=category, price=price, price_currency=price_currency,
        )
...
async def get_company_knowledge(db, company_id: str | None = None, current_query: str = "", top_k: int = 5) -> str:
    try:
        ...
        logger.warning(
            "ai_facade get_company_knowledge local failed company_id=%s error=%s",
            company_id or "", exc.__class__.__name__,
        )
        return ""
...
async def get_active_llm_engines(db, company_id: str = "") -> list[dict]:
    try:
        ...
        logger.warning(
            "ai_facade get_active_llm_engines local failed error=%s",
...
async def get_active_llm_engine(db, company_id: str = "") -> Optional[dict]:
    try:
        ...
        logger.warning(
            "ai_facade get_active_llm_engine local failed error=%s",
            exc.__class__.__name__,
        )
        return None
