...
class SupportAgent(BaseAgent):
    ...
    async def _handle_message_support(
        ...
        # When the caller has elected to source the response from the new
        # conversation engine, skip the legacy generation entirely. The other
        # agents (capture/qualification/analytics) still ran upstream of this
        # one so sentiment/intent/qualification metadata is already on
        # context.agent_outputs.
        if getattr(request, "suppress_response_generation", False):
            return AgentRunResult(
                ...
                payload={
                    ...
                    "escalate": False,
                    "next_action": "conversation_engine_override",
                },
            )
        # Wave 6+: this path is only reached when the per-company opt-out is
        # set (ai_use_conversation_engine = FALSE). Log it so we can measure
        # the actual opt-out rate in production before deleting the legacy
        # generator. A week of zero hits is the signal to proceed with
        # deletion.
        logger.warning(
            "legacy_support_agent_message_path_executed company_id=%s conversation_id=%s "
            "channel=%s reason=engine_opt_out workflow_id=%s",
            str(getattr(request, "company_id", "") or ""),
