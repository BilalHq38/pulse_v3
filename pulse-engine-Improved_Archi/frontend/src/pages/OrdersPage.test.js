...
// Wave 5: shipped/delivered transition buttons appear at the right times.

function installApiMocksWithStatus(initialStatus) {
  const order = { ...ORDER, status: initialStatus };
  mockApi.get.mockImplementation((url) => {
    if (url === '/orders') return Promise.resolve({ data: { items: [order] } });
    if (url === '/orders/order-1') return Promise.resolve({ data: order });
    return Promise.resolve({ data: {} });
  });
  mockApi.patch.mockImplementation((_url, payload) => Promise.resolve({ data: { ...order, status: payload.status } }));
}

test('confirmed orders expose the Mark shipped transition', async () => {
  installApiMocksWithStatus('confirmed');
  const { container, root } = await renderPage();

  const shipBtn = await waitForSelector('button[aria-label="Mark shipped"]', container);
  await act(async () => {
    shipBtn.dispatchEvent(new MouseEvent('click', { bubbles: true }));
  });
  await tick();
  expect(mockApi.patch).toHaveBeenCalledWith('/orders/order-1/status', { status: 'shipped' });
  await act(async () => root.unmount());
});

test('shipped orders expose the Mark delivered transition', async () => {
  installApiMocksWithStatus('shipped');
  const { container, root } = await renderPage();

  const deliveredBtn = await waitForSelector('button[aria-label="Mark delivered"]', container);
  await act(async () => {
    deliveredBtn.dispatchEvent(new MouseEvent('click', { bubbles: true }));
  });
  await tick();
  expect(mockApi.patch).toHaveBeenCalledWith('/orders/order-1/status', { status: 'delivered' });
  await act(async () => root.unmount());
});

test('delivered status renders an emerald badge label', async () => {
  installApiMocksWithStatus('delivered');
  const { container, root } = await renderPage();

  expect(container.textContent).toContain('Delivered');
  // No Mark-shipped affordance for already-delivered orders.
  expect(container.querySelector('button[aria-label="Mark shipped"]')).toBeNull();
  await act(async () => root.unmount());
});
