...
function ResponseStyleTab({ isAdmin }) {
  ...
  const setDefault = useCallback(async (id) => {
    ...
    try {
      ...
      await loadTemplates();
      showToast({ type: 'success', title: 'Default template updated', message: 'New replies will use this style.' });
    } catch (err) {
      showToast({
        type: 'error',
        title: 'Could not update default',
        message: getErrorMessage(err, 'We could not change the default template.'),
      });
    } finally {
      setBusyId('');
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">Response style</h2>
        <p className="mt-1 text-sm text-slate-500">
          Choose how the AI replies feel. The default template is injected into every
          conversation; agents can override per-conversation later.
        </p>
      </div>
      {loading ? (
          ...
          {templates.map((tmpl) => (
            <div
              ...
              className={`rounded-xl border p-4 ${
                tmpl.is_default ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-white'
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-sm font-semibold text-slate-900">{tmpl.name}</h3>
                {tmpl.is_default ? (
                  <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-100 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-emerald-800">
                    Default
                  </span>
                ) : isAdmin ? (
                  <button
...
export default function SettingsPage() {
  ...
  const tabs = [
    ...
    { id: 'response-style', label: 'Response Style', icon: Bot },
      ...
      <div className={`flex ${standaloneUnification ? '' : 'gap-8'} h-[calc(100vh-8rem)] overflow-hidden`}>
        ...
        <div className={`${standaloneUnification ? 'w-full' : 'flex-1 min-w-0 overflow-y-auto'}`}>
          ...
          {activeTab === 'response-style' && (
            <ResponseStyleTab isAdmin={isAdmin} />
          )}
