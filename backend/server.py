...
async def start_followup_scheduler_loop() -> None:
    # Run the proactive-follow-up dispatcher in-process. Co-locating it with
    # the API server avoids needing a separate worker service for v1; once
    # follow-up volume justifies isolation, this hook can move into a
    # dedicated services/followup_worker entry point with the same code path.
    from services.followup_scheduler import start_followup_scheduler

    try:
        app.state.followup_scheduler_handle = await start_followup_scheduler(app.state.db)
    except Exception as exc:
        # The API server must come up even if the follow-up loop can't start
        # (e.g. DB still warming up). The loop will simply not run until the
        # next deploy.
        app.state.followup_scheduler_handle = None
        import logging
        logging.getLogger(__name__).warning("followup_scheduler_start_failed error=%s", exc)


@app.on_event("shutdown")
async def stop_followup_scheduler_loop() -> None:
    from services.followup_scheduler import stop_followup_scheduler

    await stop_followup_scheduler(getattr(app.state, "followup_scheduler_handle", None))
