"""AI service management endpoints for engines, agents, MCP, social, and webhooks."""
...
def _tenant_resource_company_id(current_user: dict, body: dict) -> str:
    if _is_super_admin(current_user):
        return (body.get("company_id", "") or "").strip()
...
async def ai_architecture(request: Request):
    ...
    return {
        ...
        "mcp_compliant": provider == "mcp",
        "action_required": (
            "Set AI_PROVIDER=mcp for strict MCP enforcement" if require_mcp and provider != "mcp" else "none"
        ),
    }


async def _fetch_scoped_llm_engine(
    db,
    llm_id: str,
    *,
    company_id: str,
    ...
    if write_access:
        return r(
            await db.fetchrow(
                "SELECT * FROM llm_engines WHERE id=$1 AND company_id=$2",
                llm_id,
                company_id,
            )
        )
    if allow_global:
        return r(
            await db.fetchrow(
                "SELECT * FROM llm_engines WHERE id=$1 AND (company_id='' OR company_id=$2)",
                llm_id,
                company_id,
            )
        )
...
async def _fetch_scoped_mcp_server(
    db,
    server_id: str,
    *,
    company_id: str,
    ...
    if is_super_admin:
        return r(await db.fetchrow("SELECT * FROM mcp_servers WHERE id=$1", server_id))
    if write_access:
        return r(
            await db.fetchrow(
                "SELECT * FROM mcp_servers WHERE id=$1 AND company_id=$2",
                server_id,
                company_id,
            )
        )
    if allow_global:
        return r(
            await db.fetchrow(
                "SELECT * FROM mcp_servers WHERE id=$1 AND (company_id='' OR company_id=$2)",
                server_id,
                company_id,
            )
        )
...
async def delete_llm_engine(llm_id: str, request: Request):
    ...
    cid = (current_user.get("company_id", "") or "").strip()
    res = (
        await db.execute("DELETE FROM llm_engines WHERE id=$1", llm_id)
        if _is_super_admin(current_user)
        else await db.execute(
            "DELETE FROM llm_engines WHERE id=$1 AND company_id=$2",
            llm_id,
            cid,
        )
    )
...
async def list_ai_embeddings(request: Request, memory_id: Optional[str] = None):
    db = _db(request)
    cu = await get_current_user_flexible(request)
    cid = cu.get("company_id", "")
    if memory_id:
        return rs(
            await db.fetch(
                "SELECT * FROM ai_embeddings WHERE company_id=$1 AND memory_id=$2 ORDER BY created_at DESC LIMIT 200",
                cid,
                memory_id,
            )
        )
    return rs(
        await db.fetch(
            "SELECT * FROM ai_embeddings WHERE company_id=$1 ORDER BY created_at DESC LIMIT 200",
            cid,
        )
    )
...
async def create_ai_embedding(request: Request):
    db = _db(request)
    cu = await get_current_user_flexible(request)
    cid = cu.get("company_id", "")
    body = await request.json()
    emb = body.get("embedding_vector") or []
    if not isinstance(emb, list):
        raise HTTPException(400, "embedding_vector must be a list")
    eid = make_id()
    await db.execute(
        "INSERT INTO ai_embeddings(id,company_id,memory_id,embedding_model,dimension,embedding_vector,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,NOW(),NOW())",  # noqa: E501
        eid,
        cid,
        body.get("memory_id", ""),
        body.get("embedding_model", ""),
        int(body.get("dimension") or len(emb)),
        emb,
    )
    return r(await db.fetchrow("SELECT * FROM ai_embeddings WHERE id=$1", eid))
...
async def list_mcp_servers(request: Request):
    ...
    servers = rs(
        await db.fetch(
            "SELECT * FROM mcp_servers ORDER BY CASE WHEN company_id='' THEN 0 ELSE 1 END, last_heartbeat DESC LIMIT 100"  # noqa: E501
        )
        if _is_super_admin(current_user)
        else await db.fetch(
            "SELECT * FROM mcp_servers "
            "WHERE company_id='' OR company_id=$1 "
            "ORDER BY CASE WHEN company_id='' THEN 0 ELSE 1 END, last_heartbeat DESC LIMIT 100",
            cid,
        )
    )
    for s in servers:
...
async def create_mcp_server(request: Request):
    ...
    current_user = await require_roles(request, ["admin", "super_admin"])
    body = await request.json()
    server_company_id = _tenant_resource_company_id(current_user, body)
    sid = make_id()
    await db.execute(
        "INSERT INTO mcp_servers(id,company_id,endpoint,status,region,last_heartbeat,created_at,updated_at) VALUES($1,$2,$3,$4,$5,NOW(),NOW(),NOW())",  # noqa: E501
        sid,
        server_company_id,
...
async def update_mcp_server(server_id: str, request: Request):
    ...
    if not body and caps is None:
        raise HTTPException(400, "No valid fields provided")
    server = await _fetch_scoped_mcp_server(
        db,
        server_id,
        company_id=cid,
        is_super_admin=_is_super_admin(current_user),
...
async def delete_mcp_server(server_id: str, request: Request):
    ...
    res = (
        ...
        if _is_super_admin(current_user)
            ...
            "DELETE FROM mcp_servers WHERE id=$1 AND company_id=$2",
            server_id,
            cid,
        )
    )
...
async def create_mcp_client(request: Request):
    ...
    body = await request.json()
    cid_new = make_id()
    if body.get("server_id") and not await _fetch_scoped_mcp_server(
        db,
        body.get("server_id", ""),
...
@router.get("/mcp/api-endpoints")
async def list_api_endpoints(request: Request, server_id: Optional[str] = None):
    db = _db(request)
    current_user = await require_roles(request, ["admin", "super_admin"])
    cid = (current_user.get("company_id", "") or "").strip()
    ...
    if server_id:
        ...
        return rs(
            await db.fetch(
                "SELECT * FROM api_endpoints WHERE server_id=$1 ORDER BY created_at DESC LIMIT 200",
                server_id,
            )
        )

    if is_super:
        return rs(await db.fetch("SELECT * FROM api_endpoints ORDER BY created_at DESC LIMIT 200"))

    return rs(
        await db.fetch(
            "SELECT ae.* FROM api_endpoints ae "
            "JOIN mcp_servers ms ON ms.id=ae.server_id "
            "WHERE ms.company_id='' OR ms.company_id=$1 "
...
@router.post("/mcp/api-endpoints")
async def create_api_endpoint(request: Request):
    db = _db(request)
    current_user = await require_roles(request, ["admin", "super_admin"])
    cid = (current_user.get("company_id", "") or "").strip()
    body = await request.json()
    ...
    endpoint_id = make_id()
    await db.execute(
        "INSERT INTO api_endpoints(id,server_id,endpoint_path,http_method,endpoint_type,request_schema,response_schema,is_active,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NOW(),NOW())",  # noqa: E501
        endpoint_id,
        server_id,
        (body.get("endpoint_path", "") or "").strip(),
        (body.get("http_method", "POST") or "POST").upper(),
        (body.get("endpoint_type", "") or "").strip(),
        body.get("request_schema", {}) if isinstance(body.get("request_schema", {}), dict) else {},
        body.get("response_schema", {}) if isinstance(body.get("response_schema", {}), dict) else {},
        bool(body.get("is_active", True)),
    )
    return r(await db.fetchrow("SELECT * FROM api_endpoints WHERE id=$1", endpoint_id))


@router.get("/mcp/client-requests")
async def list_client_requests(
    ...
    if client_id:
        return rs(
            await db.fetch(
                "SELECT * FROM client_requests WHERE company_id=$1 AND client_id=$2 ORDER BY requested_at DESC LIMIT $3",  # noqa: E501
                cid,
                client_id,
                safe_limit,
            )
    ...
    return rs(
        await db.fetch(
            "SELECT * FROM client_requests WHERE company_id=$1 ORDER BY requested_at DESC LIMIT $2",
            cid,
            safe_limit,
        )
    )
...
@router.post("/mcp/client-requests")
async def create_client_request(request: Request):
    db = _db(request)
    cu = await get_current_user_flexible(request)
    cid = cu.get("company_id", "")
    body = await request.json()
    ...
    if not client_id or not endpoint_id:
        raise HTTPException(400, "client_id and endpoint_id are required")

    client = r(
        await db.fetchrow(
            "SELECT * FROM mcp_clients WHERE id=$1 AND company_id=$2",
            client_id,
            cid,
        )
    )
    ...
    endpoint = r(await db.fetchrow("SELECT * FROM api_endpoints WHERE id=$1", endpoint_id))
    if not endpoint:
        raise HTTPException(404, "API endpoint not found")

    req_id = make_id()
    await db.execute(
        "INSERT INTO client_requests(id,company_id,client_id,endpoint_id,convo_id,message_id,response_status,requested_at,responded_at,created_at,updated_at,raw_data) VALUES($1,$2,$3,$4,$5,$6,$7,NOW(),NOW(),NOW(),NOW(),$8)",  # noqa: E501
        req_id,
        cid,
        client_id,
        endpoint_id,
        body.get("convo_id", ""),
        body.get("message_id", ""),
        int(body.get("response_status", 0) or 0),
        {
            "request_payload": body.get("request_payload", {})
            if isinstance(body.get("request_payload", {}), dict)
            else {},
            "response_payload": body.get("response_payload", {})
            if isinstance(body.get("response_payload", {}), dict)
            else {},
        },
    )
    return r(await db.fetchrow("SELECT * FROM client_requests WHERE id=$1", req_id))
...
async def mcp_server_heartbeat(server_id: str, request: Request):
    ...
    cid = (current_user.get("company_id", "") or "").strip()
    body = await request.json()
    if not await _fetch_scoped_mcp_server(
        db,
        server_id,
        company_id=cid,
        is_super_admin=_is_super_admin(current_user),
...
async def create_social_account(request: Request):
    ...
    await db.execute(
        ...
        said,
        cu.get("company_id", ""),
        (body.get("platform", "") or "").strip().lower(),
...
@router.get("/mcp/webhook-handlers")
...
@router.post("/mcp/webhook-handlers")
