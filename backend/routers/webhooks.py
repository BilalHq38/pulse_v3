...
_AI_IMAGE_SEND_PHRASES = (
    ...
    "attached the image",
    "product image",
    # Phrases generated by the conversation engine system prompt
    "i am sharing the product image",
    "sharing the product image",
    "sharing a product image",
    "i'm sharing",
    "attaching the image",
    "attaching an image",
    "image attached",
...
async def _send_product_images(
    ...
    *,
    ...
    # If the conversation engine already resolved image URLs via product_links, send them
    # immediately — do not gate on whether the user explicitly asked for images.
    # The engine context already confirmed these products have images in the DB.
    # Only fall back to the image-request gate when we have no pre-resolved links and
    # need to do an ad-hoc catalog lookup (legacy agent path).
    engine_links_with_images = [
        pl for pl in (product_links or [])
        if str(pl.get("image_url") or "").strip()
    ]
    if not engine_links_with_images and not _is_image_request(user_message, ai_response=ai_response):
        return []
    ...
    effective_links: list[dict] = list(engine_links_with_images) if engine_links_with_images else []

    # When product_links has items with product_ids but no image_url, do a direct
    # DB lookup by ID first. This is far more reliable than semantic search and
    # prevents wrong product images from being sent when the ranker returns similar
    # (but not exact) products for image requests.
    if not effective_links and (product_links or []):
        id_only = [pl for pl in (product_links or []) if pl.get("product_id") and not str(pl.get("image_url") or "").strip()]
        if id_only:
            try:
                from services.conversation_engine.retrieval.products import _fetch_first_images
                img_map = await _fetch_first_images(db, [pl["product_id"] for pl in id_only], company_id)
                for pl in id_only:
                    img = img_map.get(str(pl.get("product_id") or ""), "")
                    if img:
                        effective_links.append({**pl, "image_url": img})
            except Exception as _direct_exc:
                logger.debug("image_direct_id_lookup_failed error=%s", _direct_exc)

    # Last resort: semantic search fallback.  Only used when we have no product_links
    # at all (legacy agent path).  Use a larger top_k and name-filter to avoid sending
    # images for wrong products.
    if not effective_links:
        try:
            from services.conversation_engine.retrieval.products import ProductRetriever
            _search_query = str(user_message or ai_response or "")
            ...
            _query_lower = _search_query.lower()
            for _chunk in _chunks:
                _meta = _chunk.metadata or {}
                _img = str(_meta.get("image_url") or "")
                _url = str(_meta.get("public_url") or "")
                _name = str(_chunk.title or _meta.get("name") or "")
                if _img and _name and _name.lower() in _query_lower:
            ...
            # If no name-matched results, accept any image-bearing chunk (original fallback)
            if not effective_links:
                for _chunk in _chunks:
                    _meta = _chunk.metadata or {}
                    _img = str(_meta.get("image_url") or "")
