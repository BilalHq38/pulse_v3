...
def _parse_bulk_product_rows(rows: list[list], *, row_images: dict[int, list[str]] | None = None) -> tuple[list[dict], list[dict]]:
    ...
    for row_index in range(header_row_index + 1, len(rows)):
        ...
        link_cell = str(read_cell(row_values, "links", "link", "product_url") or "").strip()
        if link_cell:
            if not _HTTP_URL_RE.match(link_cell):
                errors.append({"row": row_number, "error": "Invalid links URL (must be http(s)://)."})
                continue
            payload["links"] = link_cell
        stock_cell = str(read_cell(row_values, "stock_quantity", "stock", "inventory") or "").strip()
        if stock_cell:
            try:
                stock_value = int(stock_cell)
                if stock_value < 0:
                    raise ValueError
            except ValueError:
                errors.append({"row": row_number, "error": "stock_quantity must be a non-negative integer."})
                continue
            payload["stock_quantity"] = stock_value
...
def _slugify(text: str) -> str:
    base = _SLUG_SANITIZE_RE.sub("-", str(text or "").strip().lower()).strip("-_")
    return _SLUG_COLLAPSE_RE.sub("-", base)


def _make_product_slug(name: str, product_id: str) -> str:
    base = _slugify(name)[:64].strip("-") or "product"
    suffix = "".join(ch for ch in str(product_id or "") if ch.isalnum())[:6].lower() or "row"
    return f"{base}-{suffix}"


def _make_company_slug(name: str, company_id: str) -> str:
    base = _slugify(name)[:48].strip("-") or "company"
    suffix = "".join(ch for ch in str(company_id or "") if ch.isalnum())[:6].lower() or "id"
    return f"{base}-{suffix}"


def _normalize_link(value) -> tuple[str, str]:
    text = str(value or "").strip()
    if not text:
        return "", "auto"
    if not _HTTP_URL_RE.match(text):
        raise HTTPException(400, "links must be a valid http(s) URL")
    if len(text) > 2000:
        raise HTTPException(400, "links must be 2000 characters or fewer")
    return text, "manual"


def _normalize_stock_quantity(value) -> int | None:
    if value is None or (isinstance(value, str) and not value.strip()):
        return None
    try:
        parsed = int(str(value).strip())
    except (TypeError, ValueError) as exc:
        raise HTTPException(400, "stock_quantity must be an integer") from exc
    if parsed < 0:
        raise HTTPException(400, "stock_quantity must be zero or positive")
    return parsed


def _normalize_public_page_enabled(value) -> bool:
    if value is None:
        return True
    ...
    if text in {"0", "false", "no", "off", "n"}:
        return False
    raise HTTPException(400, "public_page_enabled must be a boolean")


async def _ensure_company_slug(db, company_id: str) -> str:
    row = await db.fetchrow("SELECT slug, name FROM companies WHERE id=$1", company_id)
    if not row:
        return ""
    existing = str(row.get("slug") or "").strip() if isinstance(row, dict) else str(row["slug"] or "").strip()
    if existing:
        return existing
    name = row["name"] if isinstance(row, dict) is False else row.get("name", "")
    candidate = _make_company_slug(str(name or ""), company_id)
    try:
        await db.execute(
            "UPDATE companies SET slug=$1, updated_at=NOW() WHERE id=$2 AND BTRIM(slug)=''",
            candidate,
            company_id,
        )
    except Exception:
        logger.exception("company_slug_assignment_failed company_id=%s", company_id)
        return ""
    return candidate
...
def _normalize_product_payload_for_create(body: dict) -> tuple[dict, list[str], list[str]]:
    ...
    links_value, link_source = _normalize_link(body.get("links"))
    payload = {
        ...
        "stock_quantity": _normalize_stock_quantity(body.get("stock_quantity")),
        "public_page_enabled": _normalize_public_page_enabled(body.get("public_page_enabled")),
    }
...
def _normalize_product_payload_for_update(body: dict) -> tuple[dict, list[str] | None, list[str] | None]:
    ...
    if "links" in body:
        links_value, link_source = _normalize_link(body.get("links"))
        payload["links"] = links_value
        payload["link_source"] = link_source
    if "stock_quantity" in body:
        payload["stock_quantity"] = _normalize_stock_quantity(body.get("stock_quantity"))
    if "public_page_enabled" in body:
        payload["public_page_enabled"] = _normalize_public_page_enabled(body.get("public_page_enabled"))
...
async def _ensure_product_slug(db, product: dict) -> str:
    """Make sure the product row has a slug. Older products created before
    Wave 1 may have an empty slug; lazily backfill it on read so the public
    URL is never blank for an active product. Manual links override the
    auto-generated path but we still want a slug as an internal fallback.
    """
    if not product:
        return ""
    current = str(product.get("slug") or "").strip()
    if current:
        return current
    ...
    if not product_id:
        return ""
    candidate = _make_product_slug(name, product_id)
    try:
        await db.execute(
            "UPDATE company_products SET slug=$1, updated_at=NOW() "
            "WHERE id=$2 AND BTRIM(COALESCE(slug,''))=''",
            candidate,
            product_id,
        )
        product["slug"] = candidate
    except Exception:
        logger.exception("product_slug_backfill_failed product_id=%s", product_id)
    return candidate


async def _attach_public_url(db, products: list[dict]) -> list[dict]:
    """Compute the canonical /c/{company_slug}/product/{slug} URL for each
    product and attach it as `public_url`. Manual `links` win when present;
    otherwise we synthesise the internal path. The company slug is shared
    across products from the same tenant so we only resolve it once per
    invocation.
    """
    if not products:
        return products
    company_id = str(products[0].get("company_id") or "")
    company_slug = await _ensure_company_slug(db, company_id) if company_id else ""
    for product in products:
        manual_link = str(product.get("links") or "").strip()
        product_slug = await _ensure_product_slug(db, product)
        if manual_link and _HTTP_URL_RE.match(manual_link):
            product["public_url"] = manual_link
        elif company_slug and product_slug:
            product["public_url"] = f"/c/{company_slug}/product/{product_slug}"
        else:
            product["public_url"] = ""
    return products
...
_PRODUCT_SELECT_SQL = (
    "SELECT p.id,p.company_id,p.name,p.product_title,p.description,p.price,p.price_currency,p.category,p.product_type,p.status,"  # noqa: E501
    "p.links,p.link_source,p.slug,p.public_page_enabled,p.stock_quantity,p.created_at,p.updated_at,"
...
async def _get_product(db, product_id, cid):
    ...
    hydrated = _hydrate_product_record(prod)
    if not hydrated:
        return None
    attached = await _attach_public_url(db, [hydrated])
    return attached[0] if attached else hydrated
...
async def _list_products_page(db, cid: str, page_size: int, offset: int) -> list[dict]:
    ...
    hydrated = [_hydrate_product_record(row) for row in rows]
    return await _attach_public_url(db, [p for p in hydrated if p])
...
async def _create_product_row(db, cid: str, payload: dict, images: list[str], features: list[str]) -> str:
    pid = make_id()
    slug = _make_product_slug(payload["name"], pid)
    await db.execute(
        "INSERT INTO company_products("
        "id,company_id,name,product_title,description,price,price_currency,category,product_type,"
        "links,link_source,slug,public_page_enabled,stock_quantity,created_at,updated_at"
        ...
        payload.get("link_source", "auto") or "auto",
        slug,
        bool(payload.get("public_page_enabled", True)),
        payload.get("stock_quantity"),
    )
    await _replace_product_assets(db, pid, images=images, features=features)
    await _ensure_company_slug(db, cid)
    return pid
...
# ---------------------------------------------------------------------------
# Product relationships (Wave 4 — upsell retrieval seed).
# ---------------------------------------------------------------------------

_VALID_RELATION_KINDS = {"complementary", "accessory", "upgrade", "similar"}


@router.post("/products/relationships/bulk-upload")
async def upload_product_relationships(request: Request, file: UploadFile = File(...)):
    """Bulk-import product_relationships rows from a CSV.

    Expected columns: product_id, related_id, relation_kind, weight.
    `relation_kind` defaults to 'complementary' when missing; `weight` defaults
    to 0.5. Rows that violate the chk_product_relationship_distinct (same
    product/related) or the uq_product_relationships unique index are
    ...
    if not file:
        raise HTTPException(400, "file_required")
    raw = await file.read()
    try:
        text = raw.decode("utf-8-sig")
    ...
    if not reader.fieldnames:
        raise HTTPException(400, "csv_missing_headers")
    headers = {(h or "").strip().lower() for h in reader.fieldnames}
    missing = [h for h in ("product_id", "related_id") if h not in headers]
    if missing:
        raise HTTPException(400, f"csv_missing_columns:{','.join(missing)}")
    ...
    for raw_row in reader:
        ...
        kind = row.get("relation_kind") or "complementary"
        weight_str = row.get("weight") or "0.5"
        if not product_id or not related_id:
            skipped.append({"row": rows_scanned, "reason": "missing_ids"})
            continue
        if product_id == related_id:
            skipped.append({"row": rows_scanned, "reason": "self_relationship"})
            continue
        if kind not in _VALID_RELATION_KINDS:
            skipped.append({"row": rows_scanned, "reason": f"invalid_kind:{kind}"})
            continue
        try:
            weight = float(weight_str)
        except ValueError:
            skipped.append({"row": rows_scanned, "reason": "invalid_weight"})
            continue
        weight = max(0.0, min(1.0, weight))
        try:
            await db.execute(
                "INSERT INTO product_relationships "
                "(id, company_id, product_id, related_id, relation_kind, weight) "
                "VALUES ($1, $2, $3, $4, $5, $6) "
                "ON CONFLICT (company_id, product_id, related_id, relation_kind) DO NOTHING",
                make_id(),
                company_id,
                product_id,
                related_id,
            ...
            # FK violation (one of the product IDs doesn't exist in this tenant),
            # check-constraint violation, or other DB error. Skip the row rather
            # than abort the whole upload.
            skipped.append({"row": rows_scanned, "reason": str(exc)[:120]})
    return {
...
async def list_product_relationships(product_id: str, request: Request):
    db = _db(request)
    cu = await get_current_user_flexible(request)
    company_id = (cu.get("company_id") or "").strip()
    if not company_id:
    ...
    return rs(
        await db.fetch(
            "SELECT pr.id, pr.product_id, pr.related_id, pr.relation_kind, pr.weight, "
            "       cp.name AS related_name "
            "FROM product_relationships pr "
            "LEFT JOIN company_products cp ON cp.id = pr.related_id "
            "WHERE pr.company_id = $1 AND pr.product_id = $2 "
            "ORDER BY pr.weight DESC, pr.relation_kind",
            company_id,
            product_id,
        )
    )
