-- Wave 2: order lifecycle expansion.
-- Adds 'shipped' and 'delivered' to the orders.status CHECK and introduces a
-- per-transition audit log so the follow-up scheduler can react to state
-- changes idempotently (see uq_order_lifecycle_event_idempotent).
...
-- Replaying the same transition (e.g. webhook retry) is a no-op.
