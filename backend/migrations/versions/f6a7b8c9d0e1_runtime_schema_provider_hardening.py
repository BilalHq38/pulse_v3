"""runtime schema and provider hardening
...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE company_faqs ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ")
    op.execute("""
        UPDATE company_faqs
           SET updated_at = COALESCE(updated_at, created_at, NOW())
         WHERE updated_at IS NULL
    """)
    op.execute("ALTER TABLE company_faqs ALTER COLUMN updated_at SET DEFAULT NOW()")
    op.execute("ALTER TABLE company_faqs ALTER COLUMN updated_at SET NOT NULL")
    op.execute("""
        CREATE INDEX IF NOT EXISTS idx_company_faqs_company_updated_at
        ON company_faqs(company_id, updated_at DESC)
    """)

    op.execute("ALTER TABLE conversations ADD COLUMN IF NOT EXISTS sentiment_score NUMERIC(5,4)")
    op.execute("UPDATE conversations SET sentiment_score = 0 WHERE sentiment_score IS NULL")
    op.execute("ALTER TABLE conversations ALTER COLUMN sentiment_score SET DEFAULT 0")
    op.execute("ALTER TABLE conversations ALTER COLUMN sentiment_score SET NOT NULL")
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS idx_context_memory_dedup_company")
    op.execute("DROP INDEX IF EXISTS idx_company_faqs_company_updated_at")
