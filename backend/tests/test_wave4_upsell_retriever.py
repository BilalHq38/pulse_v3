...
def test_orchestrator_swaps_in_related_retriever_for_proactive_upsell():
    ...
    proactive post-conversation turns anchored to an order_id."""
    orch = Orchestrator()
    ...
    feedback_req = TurnRequest(
        session_id="s1", company_id="c1", user_message="ask about delivery satisfaction",
        mode="proactive", workflow_kind="post_delivery_feedback", order_id="o1",
    )
    assert isinstance(orch._retriever_for("product", feedback_req), OrderRelatedProductRetriever)
...
def test_orchestrator_end_to_end_post_delivery_feedback_uses_related_product(monkeypatch):
    captured_prompts: list[str] = []
        ...
        async def generate(self, prompt, *, engine=None):
            ...
            return GenerationResult(
                text=(
                    ...
                    "A Phone Case pairs well with your purchase if you need extra protection: "
                    "https://example.test/phone-case"
                ),
                attempts=1,
            )
    ...
    db = _FakeDb(
        ...
        relationships=[{
            ...
            "relation_kind": "complementary", "weight": 0.9,
            "name": "Phone Case", "price": "19.99", "category": "accessory",
            "slug": "phone-case", "links": "https://example.test/phone-case", "stock_quantity": 10,
            "product_title": "Phone Case", "product_type": "accessory",
            "description": "Protective phone case", "features": [],
            "price_currency": "USD",
        }],
    ...
    assert "satisfied" in result.answer.lower()
    assert "Phone Case" in result.answer
    assert any("Phone Case" in p for p in captured_prompts)
    assert any("satisfied with the product" in p for p in captured_prompts)
    assert "product" in result.sources_used
